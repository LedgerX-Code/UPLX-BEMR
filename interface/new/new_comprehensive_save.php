...
require_once("$srcdir/api/uplx.inc.php");
...
// add facility to patient
$dres = sqlQuery("select facility_id, facility from users where username = ?", array($_SESSION['authUser']));
$newdata['patient_data']['facility'] = $dres['facility'];
$newdata['patient_data']['facility_id'] = $dres['facility_id'];

//getPatientDetail("B123443222", "1585032745");
...
// sqlStatement("unlock tables");
// end table lock
$dob = new DateTime($newdata['patient_data']['DOB']);
$data_array =  array(
    "icPassport"        => $newdata['patient_data']['pubpid'],
    ...
    "patient"         => array(
        ...
        "geneticSyndromes"         => '',
        "allergies"         => '',
        "genderOfOrigin"         => $newdata['patient_data']['sex'],
        "createdDate"         => $now->getTimestamp(),
        "reasonOfDeath"         => $newdata['patient_data']['deceased_reason'],
        "dateOfDeath"         => empty($newdata['patient_data']['deceased_date']) ? 0 : (new DateTime($newdata['patient_data']['deceased_date']))->getTimestamp(),
        "variables"         => array(
            "gender"         => $newdata['patient_data']['sex'],
            "healthCard"         => '',
            "dietaryRestrictions"         => '',
            "smokingFrequency"         => '',
            ...
            "weeklyFitnessHours"         => 0,
            "createdDate"         => $now->getTimestamp()
        ),
        "location"         => array(
            "postCode"         => $newdata['patient_data']['postal_code'],
            "district"         => $newdata['patient_data']['city'],
            "state"         => $newdata['patient_data']['state'],
            "nationality"         => empty($newdata['patient_data']['county']) ? '': $newdata['patient_data']['county'],
            "birthPlace"         => '',
            "accessibilityToHospital"         => '',
            "createdDate"         => $now->getTimestamp(),
        ),
        "social"         => array(
            "socialId"         => '',
            "maritalStatus"         => $newdata['patient_data']['status'],
            "dependency"         => $newdata['patient_data']['family_size'],
            "ethnicity"         => $newdata['patient_data']['ethnicity'],
            "incomeStatus"         => '',
            "income"         => (int)$newdata['patient_data']['monthly_income'],
            "insuranceType"         => '',
            "religon"         => $newdata['patient_data']['religion'],
            "employmentStatus"         => $newdata['patient_data']['occupation'],
            "criminalBackground"         => '',
            "geneticHistory"         => '',
            "languageSpoken"         => $newdata['patient_data']['language'],
            "createdDate"         => $now->getTimestamp(),
            "educationLevel"         => ''
        )
    ),
...
createPatient(json_encode($data_array));
