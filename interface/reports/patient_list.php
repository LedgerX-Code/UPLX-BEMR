...
<form name='theform' id='theform' method='post' action='patient_list.php' onsubmit='return top.restoreSession()'>
    ...
    if ($_POST['form_refresh'] || $_POST['form_csvexport']) {
        ...
        // filter with facility
        $dres = sqlQuery("select facility_id from users where username = ?", array($_SESSION['authUser']));
        if($dres['facility_id']){
            $query .=  " WHERE p.facility_id = " . $dres['facility_id'] . " ";
        }

        $query .= "GROUP BY p.lname, p.fname, p.mname, p.pid, i1.date, i2.date " .
