...
if ($_POST['form_save']) {
    ...
    if ($issue) {
        ...
        // call to UPLX API -- Condition: Create new

        $patient_data =  sqlQuery("SELECT pubpid,DOB FROM patient_data WHERE  pid = ?", array($thispid));
        $dob = new DateTime($patient_data['DOB']);
        $now = new DateTime();
        $outCome = sqlQuery("SELECT title FROM list_options WHERE list_id = ? AND option_id = ? AND activity=1", array('outcome', $_POST['form_outcome']));
        $occurrence = sqlQuery("SELECT title FROM list_options WHERE list_id = ? AND option_id = ? AND activity=1", array('occurrence', $_POST['form_occur']));
        $data_array =  array(
            "icPassport"        => $patient_data['pubpid'],
            "dateOfBirth"        => $dob->getTimestamp(),
            "issue"         => array(
                "issueId"         => text($issue),
                "issueType"        => text($ISSUE_TYPES[$_REQUEST["thistype"]][1]),
                "title"         => $_POST['form_title'],
                "visitType"         => '',
                "visitCode"         => $thisenc,
                "consultationId"         => $thisenc,
                "issueDate"         => $now->getTimestamp(),
                "beginDate"         => empty($form_begin) ? 0 : (new DateTime($form_begin))->getTimestamp(),
                "endDate"         => empty($form_end) ? 0 : (new DateTime($form_end))->getTimestamp(),
                "occurrence"         => $occurrence['title'],
                "referredBy"         => $_POST['form_referredby'],
                "remarks"         => $_POST['form_comments'],
                "outCome"         => $outCome['title']
            ),
        );
        createIssue(json_encode($data_array));
