...
require_once $GLOBALS['srcdir'].'/api/uplx.inc.php';
...
class Form_Controller extends Abstract_Controller
{
    ...
    function doIntegration($encounterId){
        $pid = empty($_SESSION['pid']) ? 0 : $_SESSION['pid'];
        $patient_data =  sqlQuery("SELECT pubpid,DOB FROM patient_data WHERE  pid = ?", array($pid));
        $visit = sqlQuery("SELECT * FROM form_encounter WHERE encounter = ? and pid = ?", array($encounterId, $pid));
        $ires = sqlStatement("SELECT list_id FROM issue_encounter WHERE encounter = ? and pid = ?", array($encounterId, $pid));
        $dob = new DateTime($patient_data['DOB']);
        $visitCate = sqlQuery("SELECT pc_catid, pc_catname, pc_cattype
                                                       FROM openemr_postcalendar_categories
                                                       WHERE pc_catid = ? ",array($visit['pc_catid']));
        $consOutcome = '';
        $departments = array();
        while ($irow = sqlFetchArray($ires)) {
            $list_id = $irow['list_id'];
            $result = sqlQuery("SELECT * FROM lists WHERE id = ? and pid = ? and activity > 0", array($list_id, $pid));
            $iOutcome = sqlQuery("SELECT * FROM list_options WHERE list_id = ? and option_id = ? and activity > 0", array('outcome', $result['outcome']));
            $consOutcome = $consOutcome . $iOutcome['title'] . ', ';
            array_push($departments, $result['destination']);
        }
        $consultData =  array(
            "icPassport"        => $patient_data['pubpid'],
            "dateOfBirth"        => $dob->getTimestamp(),
            "consultation"         => array(
                "doctorLicenseNo"         => '',
                "orgLicenseNo"        => '',
                "orgCountry"         => '',
                "consultedDate"         => (new DateTime($visit['date']))->getTimestamp(),
                "category"         => $visitCate['pc_catname'],
                "departments"         => $departments,
                "visitCode"         => $visit['id'],
                "visitType"         => '',
                "remarks"         => $visit['reason'],
                "consOutcome"         => $consOutcome,
                "fee"         => 0,
                "id"         => $visit['id'],
        ...
        // get prescription with encounter
        $medicines_data = array();
        $ipres = sqlStatement("SELECT * FROM prescriptions WHERE encounter = ? and patient_id = ? and active > 0", array($encounterId, $pid));
        while ($prow = sqlFetchArray($ipres)) {
            $medication = array(
                "drugName"         => $prow['drug'],
                "drugType"         => '',
                "dose"         => $prow['dosage'],
                ...
                "frequency"         => '',
                "validityOfDrug"         => 0,
                "startedDate"         => empty($prow['start_date']) ? 0 : (new DateTime($prow['start_date']))->getTimestamp(),
                "endedDate"         => empty($prow['end_date']) ? 0 : (new DateTime($prow['end_date']))->getTimestamp()
            );
            array_push($medicines_data, $medication);
        }
        ...
        createConsultation(json_encode($consultData));
        createPrescription(json_encode($prescriptionData));
    }
}
